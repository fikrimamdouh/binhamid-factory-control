...
if(currentVersion<15||currentVersion>18)fail('SCHEMA_VERSION_OUT_OF_RANGE','Production schema must be between versions 15 and 18.',{currentVersion});
...
const result={ok:true,currentVersion,targetVersion:18,counts:state.counts,backup:{fileName:manifest.fileName,checksumSha256:manifest.checksumSha256,schemaVersion:Number(manifest.schemaVersion),encrypted:true}};
writeFileSync(resultPath,`${JSON.stringify(result,null,2)}\n`,{mode:0o600});console.log(`[governance-preflight] READY ${currentVersion}->18`);
