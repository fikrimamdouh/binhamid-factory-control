...
'operationalRecords',to_regclass('public.operational_records') is not null,
'notificationOutbox',to_regclass('public.notification_outbox') is not null),
...
if(currentVersion<18||currentVersion>targetVersion)fail('SCHEMA_VERSION_OUT_OF_RANGE',`Production schema must be between versions 18 and ${targetVersion}.`,{currentVersion,targetVersion});
...
writeFileSync(resultPath,`${JSON.stringify(result,null,2)}\n`,{mode:0o600});console.log(`[governance-preflight] READY ${currentVersion}->${targetVersion}`);
