...
const expectedDates=['2026-07-23','2026-07-25','2026-07-26','2026-07-27'];
...
const dates=[...new Set(payments.map(row=>String(row?.reportDate||'')).filter(Boolean))].sort();
if(payments.length!==expectedRows||total!==expectedAmount||JSON.stringify(dates)!==JSON.stringify(expectedDates)){
  fail('PAYLOAD_TOTALS_INVALID','The reviewed payload totals or dates differ from the approved review.',{rows:payments.length,total,dates,expectedRows,expectedAmount,expectedDates});
}
...
if(String(process.env.REVIEWED_PAYMENT_VALIDATE_ONLY||'')==='1'){
  const output={ok:true,validatedOnly:true,hash:actualHash,rows:payments.length,total,dates};
  writeFileSync(resultPath,`${JSON.stringify(output,null,2)}\n`,{mode:0o600});
...
const output={ok:true,committed:true,source:payload.source,hash:actualHash,expected:{rows:expectedRows,amount:expectedAmount,dates:expectedDates},result,completedAt:new Date().toISOString()};
writeFileSync(resultPath,`${JSON.stringify(output,null,2)}\n`,{mode:0o600});
