...
if(manifest.databaseScope!=='public-schema'||!Array.isArray(manifest.schemas)||!manifest.schemas.includes('public'))fail('BACKUP_SCOPE_NOT_RESTORABLE','The backup is not limited to the independently restorable public application schema.',{databaseScope:manifest.databaseScope||null});
...
if(result.ok!==true||basename(String(result.file||''))!==encrypted[0]||Number(result.sizeBytes)!==size||result.checksumSha256!==checksum||result.storagePath!==null||result.databaseScope!=='public-schema')fail('BACKUP_RESULT_MISMATCH','The backup result does not match the verified encrypted public-schema file.');
if(process.env.GITHUB_ENV)appendFileSync(process.env.GITHUB_ENV,`${manifestEnvName}=${manifestPath}\n`,{encoding:'utf8',mode:0o600});
const verification={ok:true,code:'BACKUP_VERIFIED',fileName:encrypted[0],sizeBytes:size,checksumSha256:checksum,schemaVersion:Number(manifest.schemaVersion),encrypted:true,databaseScope:manifest.databaseScope,schemas:manifest.schemas,pgDumpVersion:manifest.pgDumpVersion||result.pgDumpVersion||null,manifestPath};
writeFileSync(verificationFile,`${JSON.stringify(verification,null,2)}\n`,{mode:0o600});
console.log(`[backup-verifier] VERIFIED ${encrypted[0]} (${size} bytes) schema=${verification.schemaVersion}; scope=${verification.databaseScope}`);
