...
select public.execute_unified_operation(
  jsonb_build_object(
    'operation_type','acceptance_task','entity_type','task','reference_no','OP-${run}','department','acceptance',
    'status','open','lifecycle_status','pending_review','title','اختبار المحرك الموحد','summary','اختبار منع التكرار',
    'source','acceptance','source_reference','acceptance:${run}','actor_id','acceptance-test','actor_role','admin',
    'idempotency_key','acceptance-operation:${run}','payload',jsonb_build_object('category','task','run','${run}')
  ),
  jsonb_build_array(jsonb_build_object('type','acceptance','chatId','acceptance-chat','title','اختبار','message','اختبار Outbox','dedupeKey','acceptance-outbox:${run}'))
) as operation_first \gset
...
select public.execute_unified_operation(
  ...
  jsonb_build_array(jsonb_build_object('type','acceptance','chatId','acceptance-chat','title','اختبار','message','اختبار Outbox','dedupeKey','acceptance-outbox:${run}'))
) as operation_duplicate \gset

select public.transition_unified_operation(
  (:'operation_first'::jsonb->>'operationId')::uuid,null::text,'in_progress'::text,'pending_review'::text,
  jsonb_build_object('id','acceptance-test','role','admin','source','acceptance','source_reference','acceptance-transition:${run}'),
  'اختبار تغيير الحالة'::text,jsonb_build_object('acceptance',true),'[]'::jsonb
) as operation_transition \gset
...
select public.commit_daily_report(
  ...
  jsonb_build_object(
    ...
    'treasuries','[]'::jsonb,'inventory','[]'::jsonb
  ),'acceptance-test'
...
select public.commit_daily_report(date '2099-12-31','acceptance-${run}.xlsx','HASH-${run}','CONTENT-${run}',jsonb_build_object('summary','{}'::jsonb,'sales','[]'::jsonb,'cashMovements','[]'::jsonb,'treasuries','[]'::jsonb,'inventory','[]'::jsonb),'acceptance-test') as duplicate_commit \gset
...
select json_build_object(
  ...
  'operationFirstDuplicate',coalesce((:'operation_first'::jsonb->>'duplicate')::boolean,false),
  'operationSecondDuplicate',coalesce((:'operation_duplicate'::jsonb->>'duplicate')::boolean,false),
  'operationTransitionStatus',:'operation_transition'::jsonb->>'status',
  'operationRecords',(select count(*) from public.operational_records where idempotency_key='acceptance-operation:${run}'),
  'operationEvents',(select count(*) from public.operation_events where operation_record_id=(:'operation_first'::jsonb->>'operationId')::uuid),
  'operationOutbox',(select count(*) from public.notification_outbox where dedupe_key='acceptance-outbox:${run}'),
  'batchId',:'batch_id','importId',:'import_id',
  ...
  'claimOne',coalesce((:'claim_one'::jsonb->>'claimed')::boolean,false),'claimTwo',coalesce((:'claim_two'::jsonb->>'claimed')::boolean,false),'claimThree',coalesce((:'claim_three'::jsonb->>'claimed')::boolean,false),
  ...
  'unbalancedEntries',(select unbalanced_entries from public.accounting_integrity_report),'entriesWithoutLines',(select entries_without_lines from public.accounting_integrity_report)
)::text;
rollback;
`;
...
try{
  ...
  const lines=String(result.stdout||'').split(/\r?\n/).map(line=>line.trim()).filter(line=>line.startsWith('{')&&line.endsWith('}'));if(!lines.length)fail('ACCEPTANCE_RESULT_MISSING','The isolated database acceptance result is missing.');
  const evidence=JSON.parse(lines.at(-1)),blockers=[];
  if(Number(evidence.schemaVersion)!==25)blockers.push('schema_version');
  if(evidence.operationFirstDuplicate||!evidence.operationSecondDuplicate)blockers.push('operation_idempotency');
  if(Number(evidence.operationRecords)!==1||Number(evidence.operationOutbox)!==1||Number(evidence.operationEvents)<3)blockers.push('operation_atomicity');
  if(evidence.operationTransitionStatus!=='in_progress')blockers.push('operation_transition');
  if(evidence.firstDuplicate)blockers.push('first_commit_marked_duplicate');if(!evidence.secondDuplicate)blockers.push('duplicate_commit_not_detected');
  if(Number(evidence.journalCount)!==3||Number(evidence.journalLines)!==6)blockers.push('journal_count');if(Number(evidence.journalDebit)!==3300||Number(evidence.journalCredit)!==3300)blockers.push('journal_balance');
  if(Number(evidence.salesOrders)!==2||Number(evidence.collections)!==1)blockers.push('operational_projection');if(Number(evidence.customerBalance)!==2700)blockers.push('customer_balance');if(!evidence.importPosted)blockers.push('import_not_posted');
  if(!evidence.claimOne||evidence.claimTwo||evidence.claimThree)blockers.push('telegram_idempotency');if(!evidence.reversalCreated||String(evidence.reversalBalanced)!=='true')blockers.push('journal_reversal');if(Number(evidence.unbalancedEntries)!==0||Number(evidence.entriesWithoutLines)!==0)blockers.push('accounting_integrity');
  const report={ok:blockers.length===0,checkedAt:new Date().toISOString(),code:blockers.length?'FINAL_DATABASE_ACCEPTANCE_FAILED':'FINAL_DATABASE_ACCEPTANCE_PASSED',transactionRolledBack:true,productionDataTouched:false,blockers,evidence};writeFileSync(outputPath,`${JSON.stringify(report,null,2)}\n`,{mode:0o600});
  if(blockers.length){console.error(`[final-db-acceptance] blockers=${blockers.join(',')}`);process.exit(1);}console.log(`[final-db-acceptance] PASSED operation=${evidence.operationRecords}; batch=${evidence.batchId}; debit=${evidence.journalDebit}; credit=${evidence.journalCredit}`);
