...
function physicalRoutes(dir='api',prefix='/api'){
  const base=path.join(root,dir);
  if(!fs.existsSync(base))return[];
  return fs.readdirSync(base,{withFileTypes:true}).flatMap(entry=>{
    if(entry.name.startsWith('_'))return[];
    const full=path.join(base,entry.name),url=`${prefix}/${entry.name.replace(/\.(?:mjs|cjs|js|ts)$/,'')}`;
    if(entry.isDirectory())return physicalRoutes(path.join(dir,entry.name),`${prefix}/${entry.name}`);
    return entry.isFile()&&/\.(?:mjs|cjs|js|ts)$/.test(entry.name)?[url]:[];
  });
}

const vercel=JSON.parse(fs.readFileSync(path.join(root,'vercel.json'),'utf8'));
const rewrites=new Map((vercel.rewrites||[]).map(item=>[item.source,item.destination]));
const physical=new Set(physicalRoutes());
const available=new Set([...physical,...rewrites.keys()]);
...
console.log('API_CONTRACTS_START');
for(const route of required)console.log(`${route} -> ${rewrites.get(route)||route}`);
console.log('API_CONTRACTS_END');
console.log(`API_PHYSICAL_ROUTES=${physical.size}`);
console.log(`API_REWRITES=${rewrites.size}`);
console.log(`API_REFERENCED_ROUTES=${references.size}`);
if(missing.length||broken.length){
  if(missing.length)console.error(`Missing required routes: ${missing.join(', ')}`);
  if(broken.length)console.error(`Broken referenced routes: ${broken.join(', ')}`);
  process.exit(1);
}
