...
async function fetchMessages(query){
  ...
  try{return await select('telegram_messages',`${query}&select=${modern}`);}catch{
...
async function conversationsView(req,res,p){
  const chatId=String(p.get('chatId')||'').trim(),search=normalize(p.get('q')),limit=clamp(p.get('limit'),1,1000,300),before=String(p.get('before')||'').trim();
  if(chatId){
    ...
    let rows=(await fetchMessages(query)||[]).map(cleanMessage);if(search)rows=rows.filter(x=>normalize(`${x.text} ${x.transcription} ${x.file_name} ${x.sender_name}`).includes(search));rows.reverse();
    return json(res,200,{ok:true,chat_id:chatId,messages:rows,next_before:rows.length?rows[0].created_at:null});
  }
  const rows=(await fetchMessages(`order=created_at.desc&limit=${limit}`)||[]).map(cleanMessage);let threads=buildThreads(rows);if(search)threads=threads.filter(x=>normalize(`${x.display_name} ${x.external_user_id} ${x.role} ${x.last_message}`).includes(search));
  return json(res,200,{ok:true,threads,total:threads.length,source_messages:rows.length});
}
async function operationsView(res,p){
  const department=String(p.get('department')||''),status=String(p.get('status')||''),entityType=String(p.get('entityType')||''),limit=clamp(p.get('limit'),1,500,100),filters=[];
  ...
  const records=await select('operational_records',query),[sales,purchases,tasks,quality,collections]=await Promise.all([
  ...
  ]),open=value=>!['closed','completed','cancelled','collected','rejected'].includes(String(value||''));
  return json(res,200,{ok:true,counts:{sales_open:(sales||[]).filter(x=>open(x.status)).length,sales_overdue:(sales||[]).filter(x=>open(x.status)&&x.delivery_date&&x.delivery_date<new Date().toISOString().slice(0,10)).length,purchase_open:(purchases||[]).filter(x=>open(x.status)).length,purchase_urgent:(purchases||[]).filter(x=>open(x.status)&&['urgent','critical'].includes(x.urgency)).length,tasks_open:(tasks||[]).filter(x=>open(x.status)).length,tasks_overdue:(tasks||[]).filter(x=>open(x.status)&&x.due_at&&new Date(x.due_at)<new Date()).length,quality_open:(quality||[]).filter(x=>open(x.status)).length,collections_total:(collections||[]).reduce((sum,x)=>sum+Number(x.amount||0),0)},records:records||[]});
}
async function databaseView(res){
  const results=await Promise.all(REQUIRED_TABLES.map(async table=>{try{await select(table,'select=*&limit=1');return{table,ready:true};}catch(error){return{table,ready:false,error:error.message};}})),missing=results.filter(x=>!x.ready);
  return json(res,200,{ok:missing.length===0,ready:missing.length===0,total:results.length,available:results.filter(x=>x.ready).map(x=>x.table),missing});
}
async function overview(res){
  const today=new Date().toISOString().slice(0,10),[imports,approvals,discrepancies,groups,users,messages]=await Promise.all([
  ...
  ]),normalizedUsers=(users||[]).map(x=>({external_id:x.external_id,external_username:x.external_username,active:Boolean(x.active&&x.app_users?.active),full_name:x.app_users?.full_name||'',role:x.app_users?.role||'pending'}));
  return json(res,200,{ok:true,counts:{pendingImports:(imports||[]).filter(x=>!['approved','rejected','opened_in_program'].includes(x.status)).length,openApprovals:(approvals||[]).filter(x=>x.status==='pending').length,openDiscrepancies:(discrepancies||[]).length,messagesToday:(messages||[]).length},imports:(imports||[]).map(x=>({...x,status_label:label[x.status]||x.status})),approvals:approvals||[],groups:groups||[],users:normalizedUsers});
}
export default async function handler(req,res){
  if(!method(req,res,['GET']))return;
  try{requireAdmin(req);const p=params(req),view=String(p.get('view')||'overview');if(view==='conversations')return conversationsView(req,res,p);if(view==='operations')return operationsView(res,p);if(view==='database')return databaseView(res);return overview(res);}catch(error){errorResponse(res,error);}
}
