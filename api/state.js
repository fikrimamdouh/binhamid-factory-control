...
function requestParams(req){return new URL(req.url||'/api/state',`https://${String(req.headers?.host||'localhost')}`).searchParams;}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){
      const metaOnly=['1','true','yes'].includes(String(requestParams(req).get('meta')||'').toLowerCase());
      const rows=await select('app_state',`key=eq.primary&select=${metaOnly?'key,revision,updated_at,updated_by,device_id':'key,revision,updated_at,updated_by,device_id,payload'}&limit=1`),row=rows?.[0];
      if(!row)return json(res,200,{revision:0,payload:null,hasState:false,metaOnly});
    ...
    // الأرصدة الافتتاحية تُنقل إلى جدولها المستقل ثم تُستبعد من حمولة الحالة عمدًا
    // (customerOpeningBalancesExternalized). في هذه الحالة خلوّها من الحمولة أمرٌ
    // متوقّع لا فقدان بيانات، فلا يجوز أن يحجبه حارس الحماية وإلا رُفض كل حفظ.
    const opsExternalized=input.payload?.ops?.customerOpeningBalancesExternalized===true;
    const incomingEmptyGroup=!(input.payload?.legacy?.cli||[]).length||(!opsExternalized&&!(input.payload?.ops?.customerOpeningBalances||[]).length);
    if(input.force!==true&&incomingEmptyGroup){let current=null,checkFailed=false;try{const rows=await select('app_state','key=eq.primary&select=clients:payload->legacy->cli,opening:payload->ops->customerOpeningBalances&limit=1'),row=rows?.[0]||{};current={legacy:{cli:row.clients},ops:{customerOpeningBalances:row.opening}};}catch(error){checkFailed=true;console.warn('[state guard]',String(error?.message||error).slice(0,140));}if(checkFailed)current=null;const groups=[['legacy.cli','بيانات العملاء',payload=>payload?.legacy?.cli]];if(!opsExternalized)groups.push(['ops.customerOpeningBalances','الأرصدة الافتتاحية للعملاء',payload=>payload?.ops?.customerOpeningBalances]);for(const[,label,pick]of(current?groups:[])){const stored=pick(current),incoming=pick(input.payload),storedCount=Array.isArray(stored)?stored.length:0,incomingCount=Array.isArray(incoming)?incoming.length:0;if(storedCount>0&&incomingCount===0)throw Object.assign(new Error(`الحفظ متوقف لحمايتك: الجهاز الحالي لا يحتوي ${label} بينما النسخة السحابية تحتوي ${storedCount} سجلًا. اسحب النسخة الحديثة أولًا.`),{status:409,code:'EMPTY_STATE_BLOCKED'});}}
