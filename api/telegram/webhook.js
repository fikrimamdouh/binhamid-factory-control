...
async function handleText(message,group,identity,text,voicePath='',stored=null){
  ...
  if((group.department==='workshop'||role==='mechanic'||role==='admin')&&isFaultMessage(t)){
    ...
    return createMaintenanceDraft({chatId,messageId:message.message_id,identity,text:t,plate:extractPlate(t),voicePath});
  }
...
async function handleCallback(update){
  const q=update.callback_query,message=q.message,identity=await ensureIdentity(q.from),role=identity.role||'pending';
  ...
  if(action==='maint_cancel')return cancelMaintenance({...message,from:q.from},id,identity);
  ...
  if(action==='approve'){
    ...
    const result=await rpc('decide_approval',{p_approval_id:id,p_decision:'approved',p_decided_by:identity.user_id,p_note:'اعتماد من Telegram'});
  ...
  if(action==='reject'){
    ...
    await rpc('decide_approval',{p_approval_id:id,p_decision:'rejected',p_decided_by:identity.user_id,p_note:'رفض من Telegram'});
