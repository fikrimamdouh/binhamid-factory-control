...
function monthBounds(period){const [year,month]=String(period).split('-').map(Number),next=new Date(Date.UTC(year,month,1));return{start:`${period}-01`,next:next.toISOString().slice(0,10)};}
...
export function buildCustomerProfitability({sales=[],unitCosts={},customers=[]}={}){
  ...
  for(const row of sales||[]){
    ...
    if(!map.has(key))map.set(key,{key,code:code||master.customer_code||master.external_id||'',name:name||master.customer_name||'عميل غير مسمى',blockQuantity:0,concreteQuantity:0,blockCost:0,concreteCost:0,recordedSales:0,netSalesBeforeVat:0,estimatedCost:0,profit:0,marginRate:null,balance:n(master.balance||master.current_balance),invoiceCount:0,taxBasisReliable:true,missingUnitCosts:new Set(),bases:new Set()});
...
export async function loadCustomerProfitability(period){
  const costData=await loadCostDecisionData(period),economics=productEconomics(costData),unitCosts=Object.fromEntries(economics.map(row=>[row.costCenter,row.unitCost])),{start,next}=monthBounds(costData.period);
  const [sales,customers]=await Promise.all([
    pagedSelect('sales_orders',`delivery_date=gte.${start}&delivery_date=lt.${next}&select=id,reference_no,sales_type,customer_external_id,customer_name,item,quantity,unit,total_amount,status,delivery_date,created_at&order=delivery_date.asc`).catch(()=>[]),
  ...
  const rows=buildCustomerProfitability({sales,unitCosts,customers});return{period:costData.period,periodStatus:String(costData.report?.period?.status||'not_calculated'),unitCosts,rows,taxFieldsAvailable:false,disclaimer:'تكلفة الوحدة متوسط شهري فعلي. بيانات المبيعات الحالية لا تفصل الضريبة والخصم والمرتجع لكل فاتورة، لذلك الربحية تقديرية حتى استكمال حقول الأساس الضريبي.'};
}
