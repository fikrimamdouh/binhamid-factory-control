...
export async function startProductImageAssistant(message,identity){
  ...
  return sendMessage(message.chat.id,'أرسل صورة واضحة للقطعة أو الملصق أو رقمها. صوّر أكثر من زاوية عند الحاجة، واجعل الكتابة قريبة وواضحة.');
}
...
export async function handleProductImage(message,identity,buffer,mimeType='image/jpeg'){
  ...
  await sendMessage(message.chat.id,'تم استلام صورة القطعة. جارٍ قراءة الأرقام والماركة والمواصفات ثم تشغيل البحث الموسع...');
  ...
  await sendMessage(message.chat.id,`<b>نتيجة قراءة الصورة</b>\nالقطعة: <b>${esc(identified.identification)}</b>\nالأكواد المقروءة: ${copyable(identified.codes)}\nدرجة الثقة: <b>${esc(identified.confidence)}</b>\nعبارة البحث: <code>${esc(identified.query)}</code>`);
