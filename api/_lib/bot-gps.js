...
export async function getGpsFleet(){const config=settings();if(config.provider!=='traccar')throw new Error(`مزود GPS ${config.provider} يحتاج محولًا خاصًا. النسخة الحالية تدعم Traccar مباشرة.`);return traccarStatus();}
export async function sendGpsFleetStatus(chatId,query=''){
  let rows;try{rows=await getGpsFleet();}catch(error){return sendMessage(chatId,esc(error.message));}
