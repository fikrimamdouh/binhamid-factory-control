...
const sourceReference=()=>`ACC-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2,7).toUpperCase()}`;
...
async function guarded(chatId,identity,work){
  if(!await canView(identity)){await sendMessage(chatId,'مركز المحاسبة متاح فقط لمن يملك صلاحية accounting.view.');return null;}
  try{return await work();}
  catch(error){
    const reference=sourceReference(),source=String(error?.sourceTable||'المصدر المحاسبي');
    console.error('[telegram accounting unavailable]',{reference,source,code:error?.code||'',message:String(error?.message||'').slice(0,250)});
    return sendMessage(chatId,`<b>تعذرت قراءة البيانات المحاسبية</b>\n\nالعملية: قراءة <code>${esc(source)}</code>.\nالسبب: مصدر البيانات غير متاح أو أعاد خطأً غير صالح للاعتماد.\nالحالة: <b>لم يتم حفظ أو اعتماد أي أرقام، ولم يتم إصدار حكم باتزان الحسابات.</b>\nيمكن إعادة المحاولة بعد قليل.\nالمرجع: <code>${esc(reference)}</code>`,keyboard([[{text:'إعادة المحاولة',callback_data:error?.retryCallback||'ent:accounting_menu'}],[{text:'فتح مركز المحاسبة',callback_data:'ent:accounting_menu'}]]));
  }
}
async function readSelect(table,query){
  try{return{ok:true,rows:await select(table,query)||[],error:null};}
  catch(error){console.warn('[telegram accounting read]',{table,message:String(error?.message||'').slice(0,250)});return{ok:false,rows:[],error};}
}
async function safeSelect(table,query){return(await readSelect(table,query)).rows;}
async function requiredSelect(table,query,retryCallback='ent:accounting_menu'){
  const result=await readSelect(table,query);
  if(!result.ok)throw Object.assign(new Error(`تعذرت قراءة ${table}`),{code:'ACCOUNTING_SOURCE_UNAVAILABLE',sourceTable:table,retryCallback,cause:result.error});
  return result.rows;
}
...
export async function sendAccountingSummary(chatId,identity){
  return guarded(chatId,identity,async()=>{
    const [entries,trial,integrityRows]=await Promise.all([
      requiredSelect('journal_entries','select=id,status,entry_date,posted_at&order=entry_date.desc,created_at.desc&limit=1000','ent:accounting_summary'),
      requiredSelect('trial_balance','select=total_debit,total_credit,balance&limit=1000','ent:accounting_summary'),
      requiredSelect('accounting_integrity_report','select=*&limit=1','ent:accounting_summary')
    ]);
...
export async function sendTrialBalance(chatId,identity){
  return guarded(chatId,identity,async()=>{
    const rows=await requiredSelect('trial_balance','select=account_code,account_name_ar,account_type,normal_side,total_debit,total_credit,balance&order=account_code.asc&limit=1000','ent:accounting_trial');
...
export async function sendAccountingIntegrity(chatId,identity){
  return guarded(chatId,identity,async()=>{
    const row=(await requiredSelect('accounting_integrity_report','select=*&limit=1','ent:accounting_integrity'))[0];
