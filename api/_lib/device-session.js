...
// Automatic same-origin device sessions are intentionally read-only. Any write,
// approval, import, posting or user-management action must use an authenticated
// administrator/user gateway instead of the public bootstrap cookie.
export const DEVICE_CAPABILITIES=Object.freeze(['state.read','dashboard.manager','governance.view','imports.read','daily_report.view','accounting.view']);
...
const signingKey=()=>{
  ...
  return crypto.createHash('sha256').update(['binhamid-device-session-v2-read-only',config.adminToken,config.supabaseKey].join('|')).digest();
};
...
export function issueDeviceSession(req,res,deviceId){
  ...
  const now=Math.floor(Date.now()/1000),payload={v:2,mode:'read-only',deviceId:id,iat:now,exp:now+MAX_AGE_SECONDS,capabilities:DEVICE_CAPABILITIES},body=encode(payload),token=`${body}.${sign(body)}`;
...
export function readDeviceSession(req){
  ...
  if(payload?.v!==2||payload?.mode!=='read-only'||!payload.deviceId||Number(payload.exp||0)<=Math.floor(Date.now()/1000))return null;
  return payload;
}
export function requireDeviceSession(req,capability){
  ...
  if(capability&&!session.capabilities?.includes(capability))throw Object.assign(new Error('هذه العملية تتطلب تسجيل دخول مستخدم معتمد.'),{status:403,code:'AUTHENTICATED_USER_REQUIRED',capability});
