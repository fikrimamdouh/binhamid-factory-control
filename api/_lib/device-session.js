...
export function issueDeviceSession(req,res,deviceId,appUserId=''){
  ...
  const userId=clean(appUserId);if(!validAppUserId(userId))throw Object.assign(new Error('معرف المستخدم غير صالح'),{status:400,code:'DEVICE_APP_USER_INVALID'});const now=Math.floor(Date.now()/1000),payload={v:SESSION_VERSION,mode:userId?'authenticated-user':'transport-only',deviceId:id,appUserId:userId||null,iat:now,exp:now+MAX_AGE_SECONDS,capabilities:DEVICE_CAPABILITIES},body=encode(payload),token=`${body}.${sign(body)}`;
