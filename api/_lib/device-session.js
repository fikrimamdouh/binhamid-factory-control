...
const signingKey=()=>{
  ...
  return crypto.createHash('sha256').update(['binhamid-device-session-v3',config.adminToken,config.supabaseKey].join('|')).digest();
};
...
export function issueDeviceSession(req,res,deviceId,appUserId=''){
  const id=clean(deviceId),boundUser=clean(appUserId,40);if(!/^dev-[A-Za-z0-9-]{8,150}$/.test(id))throw Object.assign(new Error('معرف الجهاز غير صالح'),{status:400,code:'DEVICE_ID_INVALID'});if(!validAppUserId(boundUser))throw Object.assign(new Error('معرف مستخدم الجهاز غير صالح'),{status:400,code:'DEVICE_APP_USER_INVALID'});
  const now=Math.floor(Date.now()/1000),payload={v:SESSION_VERSION,deviceId:id,appUserId:boundUser||null,iat:now,exp:now+MAX_AGE_SECONDS,capabilities:DEVICE_CAPABILITIES},body=encode(payload),token=`${body}.${sign(body)}`;
...
export function readDeviceSession(req){
  ...
  if(payload?.v!==SESSION_VERSION||!payload.deviceId||Number(payload.exp||0)<=Math.floor(Date.now()/1000)||!validAppUserId(payload.appUserId))return null;
