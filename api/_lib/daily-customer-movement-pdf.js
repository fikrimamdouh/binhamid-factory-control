...
async function rowsForBatches(table,fields,batchIds=[]){const out=[];for(const part of chunks(batchIds)){if(!part.length)continue;out.push(...await select(table,`batch_id=in.(${part.join(',')})&select=${fields}&limit=20000`).catch(()=>[]));}return out;}
...
export async function loadCustomerMovementProjection(analysis={},reportDate=''){
  ...
  const[openingRows,batches]=await Promise.all([
    select('customer_opening_balances','select=customer_code,customer_name,balance,balance_date&limit=10000').catch(()=>[]),
    select('daily_report_batches',`status=eq.approved&report_date=gte.${fromDate}&report_date=lt.${toDate}&select=id,report_date,status&order=report_date.asc&limit=1000`).catch(()=>[])
  ...
  return{timeline,rows,totals,batches};
}
