...
import { dailyReportReviewKeyboard } from './bot-daily-report-review.js';
...
export function dailyReportApprovalDecision(recognizedDaily,status,canApprove){const ready=Boolean(recognizedDaily&&status==='ready');return{shouldPost:false,waitingApproval:ready,canApprove:ready&&Boolean(canApprove)};}
...
async function notifyDailyReportApprovers(details,excludeChatIds=[]){
  ...
  const sent=await Promise.allSettled(chats.map(chatId=>sendMessage(chatId,text,{...dailyReportReviewKeyboard(details.importId,{canApprove:true}),action_name:'daily_report_pending_approval',action_payload:{import_id:details.importId,report_date:details.reportDate}}))),delivered=sent.filter(item=>item.status==='fulfilled').length,failed=sent.length-delivered;
...
async function sendCumulativeDailyReports(chatId,analysis,name,reportDate){
  try{
    await sendMessage(chatId,'جارٍ إعداد كشف حركة العملاء وتقريري البلوك والخرسانة للمراجعة قبل الاعتماد.');
    const movement=await generateCustomerMovementPdf(analysis||{},reportDate,name);await sendDocumentBuffer(chatId,movement.pdf,movement.filename,'application/pdf',movement.caption);await relayPdfToOwner(chatId,movement.pdf,movement.filename,`نسخة إدارة — ${movement.caption}`);
    const reports=await generateCumulativeDailyPdfs(analysis||{},name,reportDate);
...
export async function handleExcel(message,group,identity,stored){
  ...
  try{
    ...
    const duplicate=(await excelStep('lookup',()=>select('imports',`file_hash=eq.${hash}&select=id,status,original_name,report_type,summary,file_path&limit=1`)))?.[0],storagePending=duplicate?.summary?.storage?.saved===false,recheck=Boolean(duplicate&&(duplicate.report_type==='unknown_excel'||duplicate.status==='failed'||!duplicate.report_type||storagePending||(dailyType(duplicate.report_type)&&!duplicate.summary?.daily?.reportDate)));
    if(duplicate&&!recheck){
      const recognizedDaily=dailyType(duplicate.report_type),statusInfo=importStatusText(duplicate.status),reportDate=duplicate.summary?.daily?.reportDate||'';
      ...
      const senderCanApprove=recognizedDaily&&!statusInfo.posted?await identityCanApproveDailyReport(identity).catch(()=>false):false;
      resultText=`هذا الملف سبق استلامه.\nالملف: <b>${esc(duplicate.original_name)}</b>\nالنوع: <b>${esc(reportTypeLabel(duplicate.report_type))}</b>\nالتاريخ التشغيلي: <b>${esc(reportDate||'غير محدد')}</b>\nحالة الاعتماد: <b>${statusInfo.label}</b>${statusInfo.callToAction}${dailySummaryText(duplicate.summary?.daily||duplicate.summary||{})}`;
      result={ok:statusInfo.posted,duplicate:true,import:duplicate,reportType:duplicate.report_type,status:duplicate.status==='failed'?'failed':'ready',recognizedDaily,reportDate,canApprove:senderCanApprove,pendingApproval:recognizedDaily&&!statusInfo.posted&&Boolean(reportDate)};
    }
    else{
      ...
      const reportType=classifyFile(name,group.department,sheetNames,contentText),recognizedDaily=dailyType(reportType),reportDate=recognizedDaily?(dailyAnalysis?.reportDate||reportDateFromFile(`${name} ${message.caption||''}`)):'',timeline=reportDate?await loadDailyReportTimeline(reportDate).catch(()=>null):null,path=duplicate?.file_path||`telegram/${group.department||'unassigned'}/${new Date().toISOString().slice(0,10)}/${hash.slice(0,16)}-${safeFile(name)}`;
      if(recognizedDaily&&dailyAnalysis){dailyAnalysis.reportDate=reportDate;dailyAnalysis.summary={...dailyAnalysis.summary,reportDate,reportDateSource:dailyAnalysis.reportDateSource||(reportDate?'filename_or_caption':'')};summary={...summary,daily:dailyAnalysis.summary,timeline};}
      ...
      const resultTitle=recheck?'تمت إعادة فحص الملف القديم وتحديث تصنيفه بنجاح.':status!=='ready'?'تم تسجيل الملف، لكن تعذر فحص محتواه آليًا.':storageSaved?'تم فحص الملف وحفظه بنجاح.':'تمت قراءة الملف وتسجيل بياناته؛ نسخة التخزين السحابي فقط معلقة.';let posting=null,postingFailure=null,senderCanApprove=false;
      if(recognizedDaily&&status==='ready'&&reportDate)senderCanApprove=await identityCanApproveDailyReport(identity).catch(error=>{console.warn('[telegram excel authorization]',{message:String(error?.message||'').slice(0,300)});return false;});
      const approval=dailyReportApprovalDecision(recognizedDaily&&Boolean(reportDate),status,senderCanApprove);
      const state=status!=='ready'?'مسجل لكن تعذر الفحص الآلي':recognizedDaily&&!reportDate?'ينتظر تحديد تاريخ الحركة':approval.waitingApproval?(storageSaved?'بانتظار الاعتماد':'بانتظار الاعتماد — نسخة التخزين معلقة'):recognizedDaily?(storageSaved?'جاهز للمراجعة':'جاهز للمراجعة — نسخة التخزين معلقة'):'جاهز للمراجعة';
      ...
      const noPostingText=recognizedDaily&&!reportDate?'تعذر تثبيت تاريخ الحركة. أعد إرسال نفس الملف مع كتابة التاريخ في التعليق بهذه الصيغة: تاريخ 20/07/2026. لم تُرحّل أي حركة.':approval.waitingApproval?'سُجل الملف دون ترحيل، وسيُخطر مالك النظام والمستخدمون المخولون بالاعتماد. راجع المسودات ثم استخدم زر الاعتماد أو الرفض.':recognizedDaily?'سُجل الملف وأصبح جاهزًا للمراجعة دون إنشاء قيود جزئية.':'لم تُرحّل البيانات نهائيًا.';
      const timelineText=timeline?`\nالرصيد الافتتاحي حتى: <b>${esc(timeline.openingDate||'غير محدد')}</b>\nالحركة من: <b>${esc(timeline.fromDate||'غير محدد')}</b> إلى: <b>${esc(timeline.toDate||reportDate)}</b>${timeline.missingDates?.length?`\nأيام غير معتمدة قبل الملف: <b>${esc(timeline.missingDates.join('، '))}</b>`:''}`:'';resultText=`${resultTitle}\n\nالاسم: <b>${esc(name)}</b>\nالنوع: <b>${esc(reportTypeLabel(reportType))}</b>\nالتاريخ التشغيلي: <b>${esc(reportDate||'غير محدد')}</b>${timelineText}\nالأوراق: ${esc(sheetNames.join('، ')||'تعذر القراءة')}\nالصفوف: <b>${rowCount}</b>${recognizedDaily?dailySummaryText(summary.daily):''}\nالحالة: <b>${state}</b>\n\n${noPostingText}${storageText}`;
      result={duplicate:false,import:imp,reportType,status,path,recognizedDaily,posting,postingFailure,storageSaved,storageFailure,reportDate,timeline,canApprove:approval.canApprove,pendingApproval:approval.waitingApproval,pendingApprovalNotice:approval.waitingApproval?{importId:imp.id,name,reportType,reportDate,summary:summary.daily||{},senderName:identity.full_name||identity.external_id}:null};
    }
  ...
  if(result?.recognizedDaily&&!result?.duplicate&&result?.reportDate&&result?.status!=='failed')result.pdfReports=await sendCumulativeDailyReports(chatId,dailyAnalysis,name,result.reportDate);
