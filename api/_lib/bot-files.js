...
export async function handleExcel(message,group,identity,stored){
  ...
  try{
    ...
    if(duplicate&&!recheck){
      ...
      if(approval.shouldPost){try{posting=await commitDailyReportFromTelegram({reportDate,originalName:name,fileHash:hash,contentHash:hash,idempotencyKey:`telegram-daily:${reportDate}:${hash}`,importId:imp.id,payload:autoPayload(dailyAnalysis,reportDate,imp.id)},String(identity.user_id||identity.external_id||'telegram-bot'));if(posting?.ok&&!posting?.duplicate)requestDailyBackup('daily-report-approved').catch(()=>null);}catch(error){postingFailure=String(error?.message||'تعذر الترحيل التلقائي').slice(0,500);console.error('[telegram excel posting]',{status:Number(error?.status||error?.upstreamStatus||0),message:postingFailure});await patch('imports',`id=eq.${encodeURIComponent(imp.id)}`,{status:'ready_for_review',last_error_code:String(error?.code||'AUTO_POSTING_FAILED').slice(0,120),last_error_message:postingFailure}).catch(()=>{});}}
