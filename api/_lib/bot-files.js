...
async function excelStep(stage,operation){
  try{return await operation();}
    ...
    const tagged=error instanceof Error?error:new Error(String(error||'Unknown error'));
    if(!tagged.excelStage)tagged.excelStage=stage;
    throw tagged;
  }
}
...
export async function handleExcel(message,group,identity,stored){
  ...
  try{
    ...
    if(duplicate&&!recheck){
      ...
      const rows=await excelStep('registry',()=>duplicate?patch('imports',`id=eq.${encodeURIComponent(duplicate.id)}`,values):insert('imports',[{source:'telegram',...values}]));
      const imp=rows?.[0]||duplicate;
