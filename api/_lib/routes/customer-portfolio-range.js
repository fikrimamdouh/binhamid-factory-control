...
export async function customerPortfolioRange(req,res){
  ...
  try{
    ...
    const today=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Riyadh',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date()),requestedTo=isoDate(req.query?.to),requestedFrom=isoDate(req.query?.from),ceiling=requestedTo||today,sector=validSector(clean(req.query?.sector)||'all'),employeeId=clean(req.query?.employee),statusFilter=clean(req.query?.status),search=norm(req.query?.search);
    ...
    for(const sale of sales||[]){const date=batchDate.get(String(sale.batch_id));if(!date||date>to)continue;const row=get(sale.customer_code,sale.customer_name),amount=number(sale.amount),type=salesType(sale);if(type)row.segment=row.segment&&row.segment!==type?'all':type;if(date<from)row.priorSales+=amount;else{row.periodSales+=amount;row.invoiceCount++;if(date>row.lastSaleDate)row.lastSaleDate=date;}}
    for(const movement of cash||[]){if(!(movement.is_customer_collection===true||String(movement.is_customer_collection)==='true'))continue;const date=batchDate.get(String(movement.batch_id));if(!date||date>to)continue;const row=get(movement.account_code,movement.account_name),amount=collectionAmount(movement);if(date<from)row.priorCollections+=amount;else{row.periodCollections+=amount;row.collectionCount++;if(date>row.lastCollectionDate)row.lastCollectionDate=date;}}
