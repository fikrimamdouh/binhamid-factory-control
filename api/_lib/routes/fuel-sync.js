...
async function mergeIntoState(rows,context){
  for(let attempt=1;attempt<=4;attempt++){
    ...
    const payload=state.payload&&typeof state.payload==='object'?state.payload:{},ops=payload.ops&&typeof payload.ops==='object'?payload.ops:{};
    const existing=Array.isArray(ops.fuel)?ops.fuel:[],seen=new Set(existing.map(rowKey)),incoming=[];
    rows.forEach((row,index)=>{const mapped=stateRow(row,{...context,index});const key=rowKey(mapped);if(!seen.has(key)){seen.add(key);incoming.push(mapped);}});
    if(!incoming.length)return{stored:0,duplicate:true,total:existing.length};
    const nextPayload={...payload,ops:{...ops,fuel:[...existing,...incoming]}};
    const revision=Number(state.revision||0),updated=await patch('app_state',`key=eq.primary&revision=eq.${revision}`,{payload:nextPayload,revision:revision+1,updated_at:new Date().toISOString()});
    if(updated?.length)return{stored:incoming.length,total:existing.length+incoming.length,revision:revision+1};
  }
...
export async function fuelDailyReport(req,res){
  ...
  try{
    const identity=await requireSyncIdentity(req),buffer=await rawBody(req,config.maxImportFileBytes);
