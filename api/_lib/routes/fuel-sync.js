...
function filename(req){
  const value=clean(req.headers?.['x-fuel-filename-b64'],1000);if(value){try{return clean(Buffer.from(value,'base64').toString('utf8'),240)||'fuel-report.xlsx';}catch{}}
...
async function jwks(){
  ...
  const response=await fetch(`${OIDC_ISSUER}/.well-known/jwks`,{headers:{Accept:'application/json'}});if(!response.ok)throw Object.assign(new Error('تعذر التحقق من هوية GitHub Actions'),{status:502,code:'GITHUB_OIDC_JWKS_FAILED'});
...
async function requireSyncIdentity(req){
  const auth=clean(req.headers?.authorization,3000);if(config.cronSecret&&auth===`Bearer ${config.cronSecret}`)return{kind:'cron-secret'};
  if(!auth.startsWith('Bearer '))throw Object.assign(new Error('هوية مزامنة الوقود مطلوبة'),{status:401,code:'FUEL_SYNC_AUTH_REQUIRED'});return{kind:'github-oidc',claims:await verifyGithubOidc(auth.slice(7))};
}
function reportDate(req,rows){
  const explicit=clean(req.headers?.['x-fuel-report-date'],20);if(/^20\d{2}-\d{2}-\d{2}$/.test(explicit))return explicit;
  const dates=rows.map(row=>clean(row.date,30).slice(0,10)).filter(value=>/^20\d{2}-\d{2}-\d{2}$/.test(value)).sort();return dates.at(-1)||riyadhDate();
}
...
function categorySummary(summary){
  const names={diesel:'الديزل',petrol:'البنزين',other:'أنواع أخرى'};return Object.entries(summary.categories||{}).filter(([,value])=>Number(value.rows||0)>0).map(([key,value])=>`${names[key]||key}: <b>${value.rows}</b> حركة، <b>${value.liters}</b> لتر، <b>${value.amount} ر.س</b>`).join('\n');
}
