...
function upstreamDetail(error){
  return clean([
    error?.message,error?.data?.message,error?.data?.details,error?.data?.hint,error?.data?.code,
    error?.storageOperation,error?.storageCode,error?.storageBucket
...
async function rawBody(req,limit){
  ...
  for await(const chunk of req){
    size+=chunk.length;
    if(size>limit)throw Object.assign(new Error('حجم ملف الوقود يتجاوز الحد المسموح'),{status:413,code:'FUEL_SYNC_FILE_TOO_LARGE'});
    chunks.push(chunk);
  }
  return Buffer.concat(chunks);
}
function filename(req){
  const value=clean(req.headers?.['x-fuel-filename-b64'],1000);
  if(value){try{return clean(Buffer.from(value,'base64').toString('utf8'),240)||'fuel-report.xlsx';}catch{}}
  return clean(req.headers?.['x-fuel-filename'],240)||'fuel-report.xlsx';
}
function requestBalance(req){
...
function requestBalanceDate(req,reportDate,accountBalance){
  if(!Number.isFinite(accountBalance))return null;
  const value=clean(req.headers?.['x-fuel-balance-date'],20);return value===reportDate?value:null;
}
...
async function jwks(){
  ...
  const data=await response.json();jwksCache={expires:Date.now()+60*60*1000,keys:Array.isArray(data.keys)?data.keys:[]};return jwksCache.keys;
}
async function verifyGithubOidc(token){
  const parts=String(token||'').split('.');
  if(parts.length!==3)throw Object.assign(new Error('رمز GitHub Actions غير صالح'),{status:401,code:'FUEL_SYNC_AUTH_REQUIRED'});
  const header=base64Json(parts[0]),claims=base64Json(parts[1]);
  if(!header||!claims||header.alg!=='RS256'||!header.kid)throw Object.assign(new Error('بنية رمز GitHub Actions غير صالحة'),{status:401,code:'FUEL_SYNC_AUTH_INVALID'});
  const key=(await jwks()).find(item=>item.kid===header.kid);
  if(!key)throw Object.assign(new Error('مفتاح GitHub Actions غير معروف'),{status:401,code:'FUEL_SYNC_AUTH_KEY_UNKNOWN'});
  ...
  if(!valid||claims.iss!==OIDC_ISSUER||!audiences(claims.aud).includes(OIDC_AUDIENCE)||claims.repository!==REPOSITORY||Number(claims.exp||0)<=now||Number(claims.nbf||0)>now+30){
    throw Object.assign(new Error('هوية GitHub Actions لا تخص مستودع مصنع بن حامد'),{status:401,code:'FUEL_SYNC_AUTH_INVALID'});
  }
  if(claims.ref!=='refs/heads/main'&&!String(claims.workflow_ref||'').includes('/.github/workflows/noor-khoy-fuel-sync.yml@refs/heads/main')){
    throw Object.assign(new Error('تشغيل مزامنة الوقود غير صادر من الفرع الرئيسي'),{status:403,code:'FUEL_SYNC_REF_FORBIDDEN'});
  }
  return claims;
}
async function requireSyncIdentity(req){
  const auth=clean(req.headers?.authorization,3000);
  if(config.cronSecret&&auth===`Bearer ${config.cronSecret}`)return{kind:'cron-secret'};
  if(!auth.startsWith('Bearer '))throw Object.assign(new Error('هوية مزامنة الوقود مطلوبة'),{status:401,code:'FUEL_SYNC_AUTH_REQUIRED'});
  return{kind:'github-oidc',claims:await verifyGithubOidc(auth.slice(7))};
}
function reportDate(req,rows){
  const explicit=clean(req.headers?.['x-fuel-report-date'],20);
  if(/^20\d{2}-\d{2}-\d{2}$/.test(explicit))return explicit;
  const dates=rows.map(row=>clean(row.date,30).slice(0,10)).filter(value=>/^20\d{2}-\d{2}-\d{2}$/.test(value)).sort();
  return dates.at(-1)||riyadhDate();
}
...
async function mergeIntoState(rows,context){
  for(let attempt=1;attempt<=4;attempt++){
    ...
    const balanceValid=Number.isFinite(context.accountBalance)&&Boolean(context.balanceCapturedAt)&&context.balanceDate===context.reportDate;
    const balanceDate=balanceValid?context.balanceDate:null,existingBalances=Array.isArray(ops.fuelBalances)?ops.fuelBalances:[];
    const balanceIndex=balanceValid?existingBalances.findIndex(item=>item?.date===balanceDate):-1,previousBalance=balanceIndex>=0?existingBalances[balanceIndex]:null;
    const balanceChanged=balanceValid&&(Number(previousBalance?.amount)!==Number(context.accountBalance)||String(previousBalance?.capturedAt)!==String(context.balanceCapturedAt));
    const nextBalances=existingBalances.slice();
    if(balanceValid&&balanceChanged){
      const snapshot={date:balanceDate,reportDate:balanceDate,amount:Number(context.accountBalance),currency:'SAR',kind:'closing',source:'noor-khoy',capturedAt:context.balanceCapturedAt};
      if(balanceIndex>=0)nextBalances[balanceIndex]=snapshot;else nextBalances.push(snapshot);
    }
    ...
    if(balanceValid){
      nextOps.fuelAccountBalance={amount:Number(context.accountBalance),currency:'SAR',kind:'closing',asOfDate:balanceDate,capturedAt:context.balanceCapturedAt,source:'noor-khoy'};
      nextOps.fuelBalances=nextBalances.sort((a,b)=>String(a.date).localeCompare(String(b.date)));
    }
    const revision=Number(state.revision||0);
    const updated=await patch('app_state',`key=eq.primary&revision=eq.${revision}`,{payload:{...payload,ops:nextOps},revision:revision+1,updated_at:new Date().toISOString()});
...
function totals(rows){
  const byCategory={};
  for(const row of rows){const key=category(row);byCategory[key]??={rows:0,liters:0,amount:0};byCategory[key].rows++;byCategory[key].liters+=Number(row.liters||0);byCategory[key].amount+=Number(row.amount||0);}
...
async function telegramDelivery({workbook,rows,sourceFile,reportDate,summary,accountBalance,duplicate}){
  ...
  try{
    ...
    for(const report of reports)await sendDocumentBuffer(config.telegramOwnerId,report.pdf,report.filename,'application/pdf',report.caption);
    return{reports:reports.map(item=>item.filename)};
  }catch(error){return{reports:[],error:clean(error?.message,300)};}
}
...
export async function fuelDailyReport(req,res){
  ...
  try{
    ...
    catch{throw Object.assign(new Error('الملف ليس Excel صالحًا'),{status:415,code:'FUEL_SYNC_EXCEL_REQUIRED'});}
    const parsed=parseFuelWorkbook(workbook,XLSX);
    if(!parsed.rowCount)throw Object.assign(new Error('لم يتم العثور على حركات وقود قابلة للقراءة داخل الملف'),{status:422,code:'FUEL_SYNC_NO_ROWS'});
    const operationalRows=parsed.rows.filter(row=>!privateFuelRow(row));
    const originalName=filename(req),hash=crypto.createHash('sha256').update(buffer).digest('hex'),date=reportDate(req,parsed.rows);
    const accountBalance=requestBalance(req),balanceCapturedAt=requestBalanceCapturedAt(req,accountBalance),balanceDate=requestBalanceDate(req,date,accountBalance),notify=requestNotify(req);
    const summary=totals(operationalRows),sourceSummary=totals(parsed.rows),context={hash,sourceFile:originalName,reportDate:date,accountBalance,balanceCapturedAt,balanceDate};
    ...
    if(existing&&['posted','approved'].includes(existing.status)){
      const state=await runStage('state_merge','تعذر تحديث بيانات الوقود',()=>mergeIntoState([],context));
    ...
    const state=await runStage('state_merge','تعذر تحديث بيانات الوقود',()=>mergeIntoState(operationalRows,context));
    const importSummary={fuel:summary,sourceFuel:sourceSummary,accountBalance,balanceDate,balanceCapturedAt,state,source:{kind:'noor-khoy',receivedAt:new Date().toISOString(),identity:identity.kind,sheets:workbook.SheetNames}};
    let imp=existing;
    if(!imp){
      ...
      const result=await runStage('import_update','تعذر تحديث ملف الوقود في مركز الوارد',()=>patch('imports',`id=eq.${encoded(imp.id)}`,{status:'approved',file_path:storagePath,row_count:operationalRows.length,summary:importSummary,last_error_code:null,last_error_message:null}));
      imp=result?.[0]||imp;
    }
