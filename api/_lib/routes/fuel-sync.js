...
function sameVehicleBalance(previous,balance){const details=previous?.details||{};return Number(details.total)===balance.amount&&Number(details.vehicle_count)===balance.vehicleCount;}
async function sendVehicleBalanceReport(req,res){
  ...
  return json(res,200,{ok:true,corrected:Boolean(previous),day,total:balance.amount,vehicleCount:balance.vehicleCount,recipients:recipients.length});
}
