...
function vehicleBalanceInput(value){const amount=Number(value?.total),accountBalance=Number(value?.accountBalance),vehicleCount=Number(value?.vehicleCount),capturedAt=new Date(clean(value?.capturedAt,80)||Date.now());if(!Number.isFinite(amount)||amount<0||amount>10_000_000)throw Object.assign(new Error('إجمالي رصيد المركبات غير صالح'),{status:400,code:'VEHICLE_BALANCE_INVALID'});if(!Number.isFinite(accountBalance)||accountBalance<0||accountBalance>10_000_000)throw Object.assign(new Error('رصيد الحساب غير صالح'),{status:400,code:'ACCOUNT_BALANCE_INVALID'});if(!Number.isInteger(vehicleCount)||vehicleCount<1||vehicleCount>5000)throw Object.assign(new Error('عدد المركبات غير صالح'),{status:400,code:'VEHICLE_BALANCE_INVALID'});if(Number.isNaN(capturedAt.getTime()))throw Object.assign(new Error('وقت قراءة الرصيد غير صالح'),{status:400,code:'VEHICLE_BALANCE_INVALID'});return{amount:Number(amount.toFixed(2)),accountBalance:Number(accountBalance.toFixed(2)),vehicleCount,capturedAt:capturedAt.toISOString()};}
async function latestVehicleBalanceReport(day){const key=`vehicle_diesel_balance:${day}`,rows=await select('audit_log',`action=eq.vehicle_diesel_balance_report_sent&entity_id=eq.${encoded(key)}&select=details,created_at&order=created_at.desc&limit=1`).catch(()=>[]);return rows?.[0]||null;}
function sameVehicleBalance(previous,balance){const details=previous?.details||{};return Number(details.total)===balance.amount&&Number(details.account_balance)===balance.accountBalance&&Number(details.vehicle_count)===balance.vehicleCount;}
async function sendVehicleBalanceReport(req,res){
  ...
  const previous=await latestVehicleBalanceReport(day);if(sameVehicleBalance(previous,balance))return json(res,200,{ok:true,duplicate:true,day});
  ...
  const results=await Promise.allSettled(recipients.map(chatId=>sendMessage(chatId,message,{action_name:'vehicle_diesel_balance_report',action_payload:{day,total:balance.amount,accountBalance:balance.accountBalance,vehicleCount:balance.vehicleCount,capturedAt:balance.capturedAt}}))),failures=results.filter(item=>item.status==='rejected').length;
  ...
  await insert('audit_log',[{actor_type:'system',actor_id:'github-actions',action:'vehicle_diesel_balance_report_sent',entity_type:'fuel_balance',entity_id:`vehicle_diesel_balance:${day}`,details:{total:balance.amount,account_balance:balance.accountBalance,vehicle_count:balance.vehicleCount,captured_at:balance.capturedAt,recipient_count:recipients.length},created_at:new Date().toISOString()}],{prefer:'return=minimal'}).catch(()=>{});
...
export async function fuelDailyReport(req,res){
  ...
  try{
    ...
    if(['vehicle-balance-report','combined-balance-report'].includes(clean(req.headers?.['x-fuel-operation'],80)))return sendVehicleBalanceReport(req,res);
