...
export const LATEST_REQUIRED_VERSION=11;
...
const COLUMN_CHECKS={
  ...
  daily_report_batches:['report_date','file_hash','content_hash','status','summary','committed_at'],
  daily_report_sales_lines:['invoice_no','sales_type','customer_code','quantity','amount'],
...
export async function collectDatabaseReadiness(){
  ...
  return{
    ready,
    schemaVersion:String(migrations.schemaVersion).padStart(3,'0'),
    latestRequiredVersion:String(LATEST_REQUIRED_VERSION).padStart(3,'0'),
    missingTables,
    missingViews,
    missingColumns,
    missingMigrations,
...
export async function databaseReadiness(req,res){
  ...
  try{
    ...
    json(res,200,{ok:database.ready, ...database, environment:{ready:environment.ready,missingRequired:environment.missingRequired,checks:environment.checks.map(({name,configured,required,description})=>({name,configured,required,description}))},nextStep:database.ready?'قاعدة البيانات متوافقة مع Migration 011.':'شغّل migrations المفقودة بالترتيب ثم أعد الفحص.'});
...
export async function status(req,res){
  ...
  json(res,200,{ok:true,version:'2026.07.17-operational-roadmap-11',...base,publicUrlConfigured:Boolean(process.env.PUBLIC_APP_URL||process.env.VERCEL_PROJECT_PRODUCTION_URL),placesConfigured:Boolean(process.env.GOOGLE_PLACES_API_KEY||process.env.PLACES_DIRECTORY_KEY),pdfConfigured:Boolean(process.env.PDF_API_URL||process.env.PDF_SERVICE_URL),webhookVersion:3,directOperationsSchema:11,conversationHistory:true,operationsActions:true,reportsCenter:true,documentVerification:true,notificationOutbox:true,schedulerWorkflow:true,dailyReportImport:true,costLedgerFoundation:true,costEngine:true,granularPermissions:true,backupTooling:true,gpsAdapter:true,vercelFunctionsExpected:6});
}
