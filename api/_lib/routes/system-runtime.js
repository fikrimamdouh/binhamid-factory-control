...
export const LATEST_REQUIRED_VERSION=17;
...
export const DATABASE_VIEWS=['daily_attendance_summary','driver_daily_summary','factory_daily_margin','cost_unit_monthly_report','control_credit_exposure','control_expiring_documents','control_open_custodies'];
...
export async function databaseReadiness(req,res){
  ...
  try{
    ...
    json(res,200,{ok:database.ready,...database,environment:{ready:environment.ready,missingRequired:environment.missingRequired,checks:environment.checks.map(({name,configured,required,description})=>({name,configured,required,description}))},nextStep:database.ready?'قاعدة البيانات متوافقة مع Migration 017.':'شغّل migrations المفقودة بالترتيب ثم أعد الفحص.'});
...
export async function status(req,res){
  ...
  json(res,200,{ok:true,version:'2026.07.17-enterprise-governance-17',...base,publicUrlConfigured:Boolean(process.env.PUBLIC_APP_URL||process.env.VERCEL_PROJECT_PRODUCTION_URL),placesConfigured:Boolean(process.env.GOOGLE_PLACES_API_KEY||process.env.PLACES_DIRECTORY_KEY),pdfConfigured:Boolean(process.env.PDF_API_URL||process.env.PDF_SERVICE_URL),webhookVersion:3,directOperationsSchema:17,conversationHistory:true,operationsActions:true,reportsCenter:true,documentVerification:true,notificationOutbox:true,schedulerWorkflow:true,dailyReportImport:true,dailyReportIdempotency:true,dailyReportCustomerMaster:true,fifoReplay:true,costLedgerFoundation:true,costEngine:true,maintenanceCostReversals:true,granularPermissions:true,backupTooling:true,gpsAdapter:true,financialPeriodClose:true,creditOverrideWorkflow:true,unifiedAssetRegister:true,complianceRegister:true,custodyControl:true,restoreTestRegister:true,handoverAcceptance:true,governanceCenter:true,vercelFunctionsExpected:6});
}
