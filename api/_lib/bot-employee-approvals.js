...
async function pendingRows(){const[rows,sessions]=await Promise.all([select('app_users','active=eq.false&select=id,external_id,full_name,role,active,employee_external_id,created_at&order=created_at.asc&limit=100'),submittedSessions()]);return(rows||[]).filter(isPendingRegistration).map(row=>({...row,registration:sessions.get(String(row.external_id))||null}));}
async function registrationById(id){const row=(await select('app_users',`id=eq.${encodeURIComponent(String(id))}&select=id,external_id,full_name,role,active,employee_external_id,created_at&limit=1`))?.[0]||null;if(!row)return null;const session=(await select('bot_sessions',`channel=eq.telegram&external_user_id=eq.${encodeURIComponent(String(row.external_id))}&select=external_user_id,state,context,updated_at&order=updated_at.desc&limit=1`))?.[0]||null;return{...row,registration:session};}
