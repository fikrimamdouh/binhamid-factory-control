...
const SNAPSHOT_VERSION='portfolio-settlement-v4-concrete-cash-bank-cutoff';
...
const emptyAging=()=>({current:0,days1to30:0,days31to60:0,days61to90:0,days90plus:0});
...
function reportCustomerCandidates(type,analyticsRows,ownershipRows,activityRows){
...
export async function generateCustomerPortfolioPdfs(analysis={},sourceFile='daily-report.xlsx',requestedTypes=['block','concrete'],options={}){
  ...
  for(const type of types){
    ...
    const analytics=await loadCustomerAnalytics({active:true,role:ANALYTICS_ROLE[type]},{asOf:reportDate,beforeDate:reportDate}),portfolio=customerRows(type,analysis,state,analytics,ownershipAnalytics,reportDate),allRows=portfolio.customers,rows=allRows.filter(row=>Number(row.finalDebt||0)>=0.5),crossSectorPurchases=portfolio.crossSectorPurchases;if(!allRows.length&&!crossSectorPurchases.length)throw Object.assign(new Error(`لا توجد مبيعات ${type==='block'?'بلوك':'خرسانة'} سابقة غير مسددة، ولا مبيعات أو سداد لهذا القسم في التقرير الحالي.`),{status:409,code:`PORTFOLIO_${type.toUpperCase()}_NO_SALES_ACTIVITY`});
    const totals=combinePortfolioTotals(aggregateSettlements(allRows),crossSectorPurchases),documentRef=`BHF-${type.toUpperCase()}-${reportDate.replace(/-/g,'')}-TG`,logoUrl=`${baseUrl}assets/branding/binhamid-factory-logo.png`,rendered=renderCustomerPortfolioDeclaration({type,companyName:state.companyName,company:state.company,employee:{name:rep?.name||'',nationalId:digits(rep?.nid||rep?.national_id),role:ROLE_BY_TYPE[type],number:rep?.no||'',phone:rep?.tel||''},customers:rows,crossSectorPurchases,days:state.days,defaultCreditLimit:state.cap,declarationText:CUSTOMER_PORTFOLIO_DECLARATION,extraText:CUSTOMER_PORTFOLIO_EXTRA,ackText:DECLARATION_ACK,authorizedName:state.authorizedName,documentRef,dateGregorian:reportDate,logoUrl,baseUrl}),crossSectorTitle=`عملاء تابعون لقطاع آخر اشتروا من ${type==='block'?'البلوك':'الخرسانة'}`,renderedDocument=rendered.document.replaceAll('مبيعات لعملاء تابعين للقطاع الآخر',crossSectorTitle),enhanced=enhancePortfolioDocument(renderedDocument,{type,rows:allRows,employee:rep,reportDate,sourceFile,logoUrl,documentRef}),document=appendSummary(enhanced,{type,rows:allRows,totals,employee:rep,reportDate,sourceFile,logoUrl,documentRef}),pdf=await htmlToPdf(document,{filename:`portfolio-${type}-${reportDate}`,landscape:false}),department=type==='block'?'البلوك':'الخرسانة',snapshot={snapshotVersion:SNAPSHOT_VERSION,crossSectorSalesMarker:CROSS_SECTOR_SALES_MARKER,crossSectorSalesIncluded:true,documentType:'customer_portfolio',portfolioType:type,reportDate,sourceFile,sourceBatchId:clean(options?.sourceBatchId),documentRef,employee:{name:rep?.name||'',nationalId:digits(rep?.nid||rep?.national_id),externalId:clean(rep?.id||rep?.external_id)},customers:rows,allCustomers:allRows,crossSectorPurchases,totals,primaryCustomerCount:rows.length,crossSectorCount:crossSectorPurchases.length,totalEntryCount:allRows.length+crossSectorPurchases.length,createdAt:new Date().toISOString()},customerCount=rows.length,totalEntryCount=allRows.length+crossSectorPurchases.length;
