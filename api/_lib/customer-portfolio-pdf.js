...
async function loadAppState(){
  ...
  return{
    declarationText:legacy?.txt?.cli||CUSTOMER_PORTFOLIO_DECLARATION,
...
function findRep(employees,type){
  ...
  return employees.find(employee=>norm(employee?.role||'').includes(wanted))||employees.find(employee=>norm(employee?.role||'').includes('مسؤول مبيعات')||norm(employee?.role||'').includes('مندوب'))||null;
}
...
export function customerPortfolioHtml({type,rows,totals,rep,state,reportDate}){
  ...
  const stillDue=rows.filter(row=>Number(row.closingBalance||0)>0);
...
export async function generateCustomerPortfolioPdfs(analysis={},sourceFile='daily-report.xlsx'){
  ...
  return Promise.all(['block','concrete'].map(async type=>{
    ...
    return{type,pdf,filename:`portfolio-${type}-${reportDate}.pdf`,caption:`${icon(type)} إقرار محفظة عملاء — ${rep?.name||ROLE_BY_TYPE[type]} — ${reportDate}`,sourceFile};
  }));
}
