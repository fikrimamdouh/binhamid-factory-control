...
function summaryPage({type,rows,totals,employee,reportDate,sourceFile,logoUrl,documentRef}){
  ...
  const alerts=rows.flatMap(row=>(row.alertLabels||[]).map(label=>({name:row.name,code:row.code,label}))).slice(0,18),cards=[['العملاء',totals.customers],['عملاء جدد',totals.newCustomers],['عملاء قدامى',totals.oldCustomers],['الرصيد السابق',money(totals.previousBalance)],['مبيعات التقرير',money(totals.reportSales)],['سداد التقرير',money(totals.reportCollections)],['المسدد من المشتريات',money(totals.paidCurrent)],['المسدد من السابق',money(totals.paidPrevious)],['الرصيد النهائي',money(totals.finalDebt)],['الدفعات المقدمة',money(totals.finalAdvance)]];
...
export async function generateCustomerPortfolioPdfs(analysis={},sourceFile='daily-report.xlsx',requestedTypes=['block','concrete'],options={}){
  ...
  for(const type of types){
    ...
    const totals=aggregateSettlements(rows),documentRef=`BHF-${type.toUpperCase()}-${reportDate.replace(/-/g,'')}-TG`,logoUrl=`${baseUrl}assets/branding/binhamid-factory-logo.png`,rendered=renderCustomerPortfolioDeclaration({type,portfolioFinancialMode:'settlement',companyName:state.companyName,company:state.company,employee:{name:rep?.name||'',nationalId:digits(rep?.nid||rep?.national_id),role:ROLE_BY_TYPE[type],number:rep?.no||'',phone:rep?.tel||''},customers:rows,days:state.days,defaultCreditLimit:state.cap,declarationText:CUSTOMER_PORTFOLIO_DECLARATION,extraText:CUSTOMER_PORTFOLIO_EXTRA,ackText:DECLARATION_ACK,authorizedName:state.authorizedName,documentRef,dateGregorian:reportDate,logoUrl,baseUrl}),document=appendSummary(rendered.document,{type,rows,totals,employee:rep,reportDate,sourceFile,logoUrl,documentRef}),pdf=await htmlToPdf(document,{filename:`portfolio-${type}-${reportDate}`,landscape:false}),department=type==='block'?'البلوك':'الخرسانة',snapshot={snapshotVersion:SNAPSHOT_VERSION,documentType:'customer_portfolio',portfolioType:type,reportDate,sourceFile,sourceBatchId:clean(options?.sourceBatchId),documentRef,employee:{name:rep?.name||'',nationalId:digits(rep?.nid||rep?.national_id),externalId:clean(rep?.id||rep?.external_id)},customers:rows,totals,createdAt:new Date().toISOString()};
...
export async function persistPortfolioReportSnapshot(report={}){
  ...
  const base=`portfolio-snapshots/${report.reportDate}/${report.type}`,pdfPath=`${base}.pdf`,snapshotPath=`${base}.json`,pointerPath=`portfolio-documents/latest-daily-${report.type}.json`,pointer={...report.snapshot,pdfPath,snapshotPath,filename:report.filename,customerCount:report.customerCount,contentType:'application/pdf'};
