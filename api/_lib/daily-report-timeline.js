...
export async function loadDailyReportTimeline(reportDate){
  ...
  const[openingRows,batches]=await Promise.all([
    select('customer_opening_balances','select=balance_date&limit=10000').catch(()=>[]),
    select('daily_report_batches','status=eq.approved&select=id,report_date,status&order=report_date.asc&limit=1000').catch(()=>[])
  ]);
  ...
  return{reportDate:target,openingDate,openingDates,firstMovementDate:openingDate?addDays(openingDate,1):'',latestApprovedDate,expectedDate,missingDates,approvedDates,fromDate:openingDate?addDays(openingDate,1):(approvedDates[0]||target),toDate:target,errors,warnings};
}
