...
export async function processNotificationOutbox(limit=50){
  ...
  for(const item of due){
    try{
      ...
      const text=`${item.title?`<b>${String(item.title).replace(/[&<>]/g,'')}</b>\n\n`:''}${String(item.message||'')}`.slice(0,3900);
  ...
  skipped=Math.max(0,due.length-sent-failed);return{processed:due.length,sent,failed,skipped};
}
export async function retryFailedNotifications(limit=30){
  const since=new Date(Date.now()-24*36e5).toISOString(),rows=await select('notification_outbox',`status=eq.failed&created_at=gte.${encodeURIComponent(since)}&select=id&order=created_at.asc&limit=${Math.max(1,Math.min(100,Number(limit)||30))}`)||[];
