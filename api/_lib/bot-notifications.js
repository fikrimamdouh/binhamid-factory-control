...
export async function sendScheduledManagerBrief(){
  if(riyadhHour()!==config.managerBriefHour)return{sent:0,reason:'outside_configured_hour',hour:riyadhHour(),configuredHour:config.managerBriefHour};
  return sendManagerBrief();
}
...
export async function sendWeeklyOperationalExport(){
  ...
  const day=riyadhDay(0),hash=crypto.createHash('sha256').update(`weekly-operational-export:${day}`).digest('hex');
  if(await alreadySent(hash,30))return{sent:0,reason:'duplicate',day};
  ...
  if(!chats.length)return{sent:0,reason:'no_managers',day};
  const payload={kind:'binhamid-operational-weekly-export',generatedAt:now(),day,notice:'تشغيلي للمراجعة؛ النسخة القابلة للاستعادة محفوظة ومشفرة في مخزن النسخ الاحتياطي.',customers,vehicles,salesOrders:orders,dailyReports:batches,maintenanceOrders:maintenance,inventoryItems:inventory,backupEvidence:backups};
  const buffer=Buffer.from(JSON.stringify(payload,null,2),'utf8');
  if(buffer.length>18*1024*1024)return{sent:0,reason:'export_too_large',bytes:buffer.length,day};
  for(const item of chats)await sendDocumentBuffer(item.external_id,buffer,`binhamid-operational-export-${day}.json`,'application/json',`تصدير تشغيلي أسبوعي — ${day}. النسخة القابلة للاستعادة تبقى مشفرة في المخزن.`);
  await markSent(hash,'weekly_operational_export',chats.length,day);return{sent:chats.length,day,bytes:buffer.length};
}
