...
export function fuelReportHtml({report,sourceFile,reportDate,category='all'}){
  const meta=CATEGORY[category]||CATEGORY.all,t=report.totals;
  ...
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><style>
    ...
    <table class="cards"><tr><td>عدد اللوحات<strong>${t.plateCount}</strong></td><td>عدد التعبئات<strong>${t.fillCount}</strong></td><td>إجمالي اللترات<strong>${qty(t.liters)}</strong></td><td>إجمالي المبلغ<strong>${money(t.amount)} ر.س</strong></td><td>حرج / تنبيه<strong>${t.danger} / ${t.warn}</strong></td></tr></table>
