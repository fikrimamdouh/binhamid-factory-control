...
export function fuelReportHtml({report,sourceFile,reportDate,category='all',accountBalance=null}){
  const meta=CATEGORY[category]||CATEGORY.all,t=report.totals,balanceCard=category==='diesel'&&Number.isFinite(accountBalance)?`<td>متبقي في الرصيد<strong>${money(accountBalance)} ر.س</strong></td>`:'';
