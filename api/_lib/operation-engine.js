...
export function buildIdempotencyKey(input={}){
  ...
  const basis={
    operationType:clean(input.operationType??input.operation_type,120),
    source:clean(input.source,30),
    sourceReference:clean(input.sourceReference??input.source_reference,240),
    referenceNo:clean(input.referenceNo??input.reference_no,120),
...
export function buildOperationEnvelope(input={}){
  const operationType=clean(input.operationType??input.operation_type,120);
  ...
  return{
    ...
    entity_type:clean(input.entityType??input.entity_type,120)||operationType,
    reference_no:clean(input.referenceNo??input.reference_no,120)||null,
    department:clean(input.department,80)||'general',
    status,
    lifecycle_status:clean(input.lifecycleStatus??input.lifecycle_status,40)||lifecycleForStatus(status),
    title:clean(input.title,500)||null,
    summary:clean(input.summary,2000)||null,
    amount:money(input.amount),
    payload,
    domain_record:input.domainRecord&&typeof input.domainRecord==='object'?input.domainRecord:{},
    source,
    source_reference:clean(input.sourceReference??input.source_reference,240)||null,
    source_chat_id:clean(input.sourceChatId??input.source_chat_id,120)||null,
    source_message_id:clean(input.sourceMessageId??input.source_message_id,120)||null,
    actor_id:clean(input.actorId??input.actor_id,120)||null,
    actor_role:clean(input.actorRole??input.actor_role,80)||null,
    created_by_user_id:clean(input.createdByUserId??input.created_by_user_id,120)||null,
    assigned_to_user_id:clean(input.assignedToUserId??input.assigned_to_user_id,120)||null,
    before_data:input.beforeData&&typeof input.beforeData==='object'?input.beforeData:{},
    after_data:input.afterData&&typeof input.afterData==='object'?input.afterData:payload,
    idempotency_key:buildIdempotencyKey(input)
  };
}

function normalizeNotifications(notifications=[],key=''){
  return(Array.isArray(notifications)?notifications:[]).filter(item=>item&&item.message).map((item,index)=>({
    type:clean(item.type,80)||'operation',
    userId:clean(item.userId,120)||null,
    chatId:clean(item.chatId,120)||null,
    title:clean(item.title,300)||null,
    message:clean(item.message,12000),
    scheduledAt:clean(item.scheduledAt,80)||null,
    dedupeKey:clean(item.dedupeKey,240)||`${key}:notification:${index+1}`,
    payload:item.payload&&typeof item.payload==='object'?item.payload:{}
  }));
}

function rpcUnavailable(error){
  return/PGRST202|Could not find the function|execute_unified_operation|transition_unified_operation/i.test(`${error?.message||''} ${JSON.stringify(error?.data||{})}`);
}
...
export async function getOperationByReference(reference){
  const value=clean(reference,120);if(!value)return null;
  return(await select('operational_records',`reference_no=eq.${encodeURIComponent(value)}&select=*&order=created_at.desc&limit=1`))?.[0]||null;
}

async function compatibilityTransition(input,notifications){
  const record=input.operationId?(await select('operational_records',`id=eq.${encodeURIComponent(input.operationId)}&select=*&limit=1`))?.[0]:await getOperationByReference(input.referenceNo);
  if(!record)throw Object.assign(new Error('العملية غير موجودة'),{status:404,code:'OPERATION_NOT_FOUND'});
  const status=clean(input.nextStatus,40)||record.status,lifecycle=clean(input.nextLifecycleStatus,40)||lifecycleForStatus(status),stamp=now(),payload={...(record.payload||{}),...(input.afterData||{}),status,lifecycle_status:lifecycle,status_note:clean(input.note,2000)};
  ...
  return{ok:true,operationId:record.id,referenceNo:record.reference_no,status,lifecycleStatus:lifecycle,outboxIds:outbox,compatibilityMode:true};
}

export async function transitionOperation(input={}){
  const notifications=normalizeNotifications(input.notifications,`transition:${clean(input.referenceNo||input.operationId,120)}:${clean(input.nextStatus,40)}`);
  const args={p_operation_id:clean(input.operationId,120)||null,p_reference_no:clean(input.referenceNo,120)||null,p_next_status:clean(input.nextStatus,40)||null,p_next_lifecycle_status:clean(input.nextLifecycleStatus,40)||null,p_actor:{id:clean(input.actorId,120)||null,role:clean(input.actorRole,80)||null,source:clean(input.source,30)||'system',source_reference:clean(input.sourceReference,240)||null},p_note:clean(input.note,2000)||null,p_after_data:input.afterData&&typeof input.afterData==='object'?input.afterData:{},p_notifications:notifications};
  try{return await rpc('transition_unified_operation',args);}
  catch(error){if(!rpcUnavailable(error))throw error;return compatibilityTransition(input,notifications);}
}

async function patchOutbox(id,values,fallback){
  try{return await patch('notification_outbox',`id=eq.${encodeURIComponent(id)}`,values);}
  catch(error){if(!fallback)throw error;return patch('notification_outbox',`id=eq.${encodeURIComponent(id)}`,fallback);}
}

export async function dispatchOperationNotifications(outboxIds=[]){
  const ids=[...new Set((Array.isArray(outboxIds)?outboxIds:[]).map(String).filter(Boolean))];
  if(!ids.length)return{queued:0,sent:0,failed:0,deadLetter:0};
  ...
  for(const row of rows){
    const attempts=Number(row.attempt_count||0)+1,started=now();
    await patchOutbox(row.id,{status:'retrying',attempt_count:attempts,last_attempt_at:started,error_text:null},{status:'retrying',error_text:null}).catch(()=>{});
    try{
      if(!row.recipient_chat_id)throw new Error('RECIPIENT_CHAT_REQUIRED');
      const options=row.payload?.telegram_options&&typeof row.payload.telegram_options==='object'?row.payload.telegram_options:{};
      await sendMessage(row.recipient_chat_id,row.message,options);
      await patchOutbox(row.id,{status:'sent',sent_at:now(),error_text:null},{status:'sent',sent_at:now(),error_text:null});result.sent++;
    }catch(error){
      const dead=attempts>=5,status=dead?'dead_letter':'failed',message=clean(error?.message||error,1000);
      await patchOutbox(row.id,{status,error_text:message,next_attempt_at:dead?null:new Date(Date.now()+Math.min(3600000,attempts*300000)).toISOString(),dead_letter_at:dead?now():null},{status,error_text:message});
      if(dead)result.deadLetter++;else result.failed++;
    }
  }
  return result;
}
