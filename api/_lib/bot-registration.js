...
async function saveName(message,identity,fullName){const value=String(fullName||'').replace(/\s+/g,' ').trim();if(value.length<3||value.length>120||/^\d+$/.test(value))return sendMessage(message.chat.id,'اكتب الاسم الكامل بصورة صحيحة، من 3 إلى 120 حرفًا.');await patch('app_users',`external_id=eq.${encodeURIComponent(String(identity.external_id||message.from.id))}`,{full_name:value});await setSession(message.chat.id,identity.external_id||message.from.id,'registration_role',{fullName:value,requestedRole:'',employeeExternalId:'',startedAt:now()});return sendMessage(message.chat.id,`تم تسجيل الاسم: <b>${esc(value)}</b>\n\nاختر الوظيفة الفعلية داخل المصنع:`,roleKeyboard());}
async function moveToConfirmation(message,identity,employeeExternalId=''){const session=await getBotSession(message.chat.id,identity.external_id||message.from.id),context=session?.context||{},employeeId=String(employeeExternalId||'').trim().slice(0,80);await patch('app_users',`external_id=eq.${encodeURIComponent(String(identity.external_id||message.from.id))}`,{employee_external_id:employeeId||null});const next={...context,employeeExternalId:employeeId};await setSession(message.chat.id,identity.external_id||message.from.id,'registration_confirm',next);return sendMessage(message.chat.id,`${sessionSummary({external_user_id:identity.external_id||message.from.id,context:next})}\n\nراجع البيانات قبل إرسالها لمدير النظام. لا تُمنح أي صلاحية قبل الاعتماد.`,confirmKeyboard());}

export async function startRegistration(message,identity){if(identity?.active)return sendMessage(message.chat.id,`حسابك معتمد بالفعل.\nالدور الحالي: <b>${esc(ROLE_LABELS[identity.role]||identity.role)}</b>\nاستخدم /menu لفتح لوحة العمليات.`);const current=await getBotSession(message.chat.id,identity.external_id||message.from.id);if(current?.state==='registration_submitted')return registrationStatus(message,identity);const telegramName=displayName(identity,message.from);await setSession(message.chat.id,identity.external_id||message.from.id,'registration_name',{telegramName,startedAt:now()});return sendMessage(message.chat.id,`<b>تسجيل موظف جديد</b>\n\nاكتب الاسم الكامل كما يظهر في سجل الموظفين.\n\nاسم Telegram الحالي: <b>${esc(telegramName)}</b>`,keyboard([[{text:'استخدام اسم Telegram',callback_data:'reg:usename'}],[{text:'عرض الوظائف أولًا',callback_data:'reg:jobs'},{text:'إلغاء',callback_data:'reg:cancel'}]]));}
...
export async function handleRegistrationCallback(message,from,identity,value){
  ...
  const session=await getBotSession(chatId,userId);if(raw==='usename')return saveName({...message,from},identity,displayName(identity,from));
