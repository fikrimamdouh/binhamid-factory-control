...
async function saveName(message,identity,fullName,presetRole=''){const value=String(fullName||'').replace(/\s+/g,' ').trim();if(value.length<3||value.length>120||/^\d+$/.test(value))return sendMessage(message.chat.id,'اكتب الاسم الكامل بصورة صحيحة، من 3 إلى 120 حرفًا.');await patch('app_users',`external_id=eq.${encodeURIComponent(String(identity.external_id||message.from.id))}`,{full_name:value});const userId=identity.external_id||message.from.id;if(presetRole&&presetRole!=='driver'){await setSession(message.chat.id,userId,'registration_employee_id',{fullName:value,requestedRole:presetRole,employeeExternalId:'',startedAt:now()});return sendMessage(message.chat.id,`تم تسجيل الاسم: <b>${esc(value)}</b>\nالوظيفة المطلوبة: <b>${esc(registrationRoleLabel(presetRole))}</b>\n\nاكتب رقم الموظف أو الكود الداخلي.`,employeeIdKeyboard());}await setSession(message.chat.id,userId,'registration_role',{fullName:value,requestedRole:'',employeeExternalId:'',startedAt:now()});return sendMessage(message.chat.id,`تم تسجيل الاسم: <b>${esc(value)}</b>\n\nاختر الوظيفة الفعلية داخل المصنع:`,roleKeyboard());}
