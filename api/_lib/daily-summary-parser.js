...
function parseDirectSales(rows,sheetName){
  ...
  while(cursor<rows.length){
    ...
    for(let index=dataStart;index<end;index++){
      ...
      sales.push({sheet:sheetName,row:index+1,reportDate:dateInRow(row)||sectionDate,invoice:String(Math.trunc(invoiceNumber)),quantity:round(quantity,3),customer,customerCode,item,kind:kind(item),amount,paymentTerms:columns.terms>=0?clean(row[columns.terms],100):''});
    }
