...
export function cumulativeDepartmentHtml({type,data,sourceFile,reportDate,latestApprovedDate,finishedGoods=[],rawMaterials=[]}){
  ...
  const customers=rows.length?`<table><thead><tr><th>#</th><th>كود العميل</th><th>العميل</th><th>الحالة</th><th>رصيد سابق</th><th>مبيعات اليوم</th><th>تحصيل اليوم</th><th>دفعة مقدمة</th><th>الرصيد المتوقع</th><th>إجمالي المبيعات</th><th>إجمالي المسدد</th></tr></thead><tbody>${rows.map((row,index)=>`<tr class="${row.closingBalance>0?'due':'clear'}"><td>${index+1}</td><td>${esc(row.code||'—')}</td><td>${esc(row.name)}</td><td>${statusBadge(row)}</td><td>${money(row.openingBalance)}</td><td>${money(row.currentSales)}</td><td>${money(row.currentApplied)}</td><td>${row.currentUnallocated>0?money(row.currentUnallocated):'—'}</td><td><strong>${money(row.closingBalance)}</strong></td><td>${money(row.cumulativeSales)}</td><td>${money(row.cumulativePaid)}</td></tr>`).join('')}</tbody></table>`:'<p class="empty">لا توجد حركة أو أرصدة لهذا القسم حتى الآن.</p>';
  const cards=[
  ...
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><style>
    ...
    *{box-sizing:border-box}
    ...
    <header class="header"><div class="badge">${icon(type)}</div><div><h1>${esc(title(type))}</h1><div class="sub">مصنع بن حامد للبلوك والخرسانة الجاهزة — تقرير تراكمي</div></div></header>
    ...
    <div class="notice"><strong>الرصيد المتوقع = الرصيد السابق + مبيعات اليوم − التحصيل الموزع.</strong> الحركة الحالية تصبح نهائية بعد اعتماد التقرير من البرنامج.</div>
