...
import { processNotificationOutbox, queueDailyReportReminders, retryFailedNotifications, sendManagerBrief, sendScheduledManagerBrief, sendWeeklyOperationalExport, sendMeaningfulAlerts } from '../_lib/bot-notifications-safe.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    else if(mode==='all')result={dailyReports:await queueDailyReportReminders(),retry:await retryFailedNotifications(),outbox:await processNotificationOutbox(),alerts:await sendMeaningfulAlerts(),brief:await sendScheduledManagerBrief(),weeklyExport:await sendWeeklyOperationalExport()};
