(function(){
  ...
  const VERSION='2026.07.22-employee-link-transfer-v2-declaration-role';
  ...
  function signalDeclarations(detail={}){try{localStorage.setItem(SIGNAL_KEY,JSON.stringify({at:new Date().toISOString(),...detail}));}catch(error){console.error('[Employee link transfer] declaration signal failed',error);}}
  ...
  async function transfer(button){
    ...
    try{
      ...
      signalDeclarations({employeeExternalId,declarationRole:result.declarationRole?.role||''});
      notify(`تم نقل حساب Telegram إلى ${result.employee?.fullName||'الموظف الجديد'}، وتحديث نموذج الخطاب حسب المسمى الوظيفي.`);
  ...
  async function updateTask(button){const user=selectedUser();if(!user)return notify('اختر مستخدم Telegram أولًا.',true);if(button)button.disabled=true;try{const result=await api({action:'update_assignment_task',appUserId:user.id,jobTitle:clean($('assignJob')?.value),shiftName:clean($('assignShift')?.value)});signalDeclarations({employeeExternalId:result.declarationRole?.employeeExternalId||'',declarationRole:result.declarationRole?.role||''});notify('تم تحديث المهمة والوردية وتحديث نموذج الخطاب المرتبط بالوظيفة.');if(typeof window.loadAll==='function')await window.loadAll(true);}catch(error){notify(`${error.message}${error.code?` [${error.code}]`:''}`,true);}finally{if(button)button.disabled=false;}}
  async function permanentDelete(employeeExternalId,name,button){if(!confirm(`سيختفي ${name} نهائيًا من قوائم الموظفين ولن تعيده ملفات Excel أو المزامنة. ستبقى السجلات التاريخية فقط. هل تريد المتابعة؟`))return;if(button)button.disabled=true;try{const result=await api({action:'permanent_delete_employee',employeeExternalId,reason:'حذف دائم من صفحة الحضور'});signalDeclarations({employeeExternalId,deleted:true});notify(`تم حذف ${name} نهائيًا من القوائم وتعطيل ${result.disabledUsers||0} حساب مرتبط.`);if(typeof window.loadAll==='function')await window.loadAll(true);}catch(error){notify(`${error.message}${error.code?` [${error.code}]`:''}`,true);}finally{if(button)button.disabled=false;}}
  function decorate(){
