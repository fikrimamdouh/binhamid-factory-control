(function(){
'use strict';
const VERSION='2026.07.21-admin-nav-v4-grouped-session-controls';
const LINKS=[
  ['🏠','البرنامج','/','التشغيل'],
...
function ensureScript(id,src,datasetName){
  if(document.getElementById(id)||document.querySelector(`script[data-${datasetName}]`))return;
  const script=document.createElement('script');script.id=id;script.src=src;script.async=false;script.dataset[datasetName.replace(/-([a-z])/g,(_,c)=>c.toUpperCase())]='1';document.head.appendChild(script);
}
function ensureOwnerLogin(){if(window.__BH_OWNER_WEB_LOGIN_INSTALLED__)return;ensureScript('bh-owner-login-loader','/assets/owner-web-login.js?v=20260721-unified-admin-session','bh-owner-login');}
...
function style(){
  ...
  const el=document.createElement('style');el.id='bh-admin-nav-style';el.textContent=`
...
function build(){
  markPage();ensureOwnerLogin();ensureSessionControls();if(document.getElementById('bhAdminNav'))return;style();
  const here=currentPath(),top=document.createElement('div');top.className='bh-admin-top';
  const logo=document.createElement('img');logo.src='/assets/branding/binhamid-factory-logo.png';logo.alt='';
  const title=document.createElement('div');title.className='t';title.innerHTML='<b>مصنع بن حامد للبلوك والخرسانة الجاهزة</b><span>تنقل إداري موحد وجلسة آمنة عبر Telegram</span>';top.append(logo,title);
  const nav=document.createElement('nav');nav.id='bhAdminNav';nav.className='bh-admin-nav';
  for(const[icon,label,href,group] of LINKS){const path=href==='/'?'/':href.replace(/\/index\.html$/,'/'),active=(here===path)||(href==='/'&&(here==='/'||here==='/index.html')),button=document.createElement('button');button.type='button';button.dataset.href=href;button.dataset.group=group;if(active)button.className='on';const i=document.createElement('span');i.className='ic';i.textContent=icon;button.append(i,document.createTextNode(label));nav.appendChild(button);}
  nav.addEventListener('click',event=>{const button=event.target.closest('button[data-href]');if(button)location.href=button.dataset.href;});
  document.body.insertBefore(nav,document.body.firstChild);document.body.insertBefore(top,nav);
}
markPage();style();ensureOwnerLogin();ensureSessionControls();window.addEventListener('binhamid-owner-authenticated',refreshCurrentPage);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',build);else build();console.info('[BinHamid]',VERSION,'loaded');
})();
