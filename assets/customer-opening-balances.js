(function customerOpeningBalances(){
  ...
  const VERSION='2026.07.19-customer-opening-balances-v3';
  ...
  function isRepeatedPageHeader(row,indexes){
    const codeLabel=normal(row[indexes.code]),nameLabel=normal(row[indexes.name]),balanceLabel=normal(row[indexes.balance]);
    if(['الاجمالي','المجموع','جمله'].includes(codeLabel)||['الاجمالي','المجموع','جمله'].includes(nameLabel))return true;
    if(codeLabel.includes('ميزان مراجعه')||codeLabel.includes('ميزان مراجعة'))return true;
    return codeLabel==='العميل'&&balanceLabel==='الرصيد';
  }
  ...
  async function parse(file){
    ...
    const endDate=reportDate(data,layout),output=[],errors=[],warnings=[],seen=new Map(),indexes=layout.indexes;let duplicatePageRows=0,ignoredPageRows=0;
    data.slice(layout.headerRow+1).forEach((row,index)=>{
      const rowNo=layout.headerRow+index+2;if(isRepeatedPageHeader(row,indexes)){ignoredPageRows++;return;}
      const customerCode=code(row[indexes.code]),customerName=String(row[indexes.name]??'').trim(),rawBalance=row[indexes.balance];
      ...
      const balance=roundMoney(balanceValue),computed=roundMoney(previous+debit-credit),difference=roundMoney(balance-computed),signature=JSON.stringify([normal(customerName),balance,roundMoney(previous),roundMoney(debit),roundMoney(credit),roundMoney(cheques)]);
      if(seen.has(customerCode)){
        if(seen.get(customerCode)===signature){duplicatePageRows++;return;}
        errors.push(`الصف ${rowNo}: كود العميل ${customerCode} مكرر بقيم مختلفة داخل الملف.`);return;
      }
      seen.set(customerCode,signature);
    ...
    return{rows:output,meta:{format:layout.format,reportDate:endDate,warnings,sourceHash:await sha256(buffer),sheetName,duplicatePageRows,ignoredPageRows}};
  }
  function preview(file,result){
    ...
    openMo('مراجعة أرصدة العملاء الافتتاحية',`<div class="note"><b>نوع الملف:</b> ${escape(formatLabel)}<br><b>تاريخ الرصيد:</b> ${escape(result.meta.reportDate)}<br>لن يُنشأ بيع أو تحصيل من الملف. الإشارة الموجبة مديونية على العميل، والسالبة رصيد دائن له. الأرصدة الافتتاحية لا تدخل أعمار الديون لعدم وجود تواريخ فواتير تفصيلية.</div><div class="stats" style="margin:12px 0"><div class="stat"><b>${plan.length}</b><span>عملاء فريدون</span></div><div class="stat"><b>${newCustomers}</b><span>جدد</span></div><div class="stat"><b>${debitTotal.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</b><span>أرصدة مدينة</span></div><div class="stat"><b>${creditTotal.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</b><span>أرصدة دائنة</span></div><div class="stat"><b>${total.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</b><span>الصافي</span></div><div class="stat"><b>${chequesTotal.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</b><span>الشيكات</span></div></div><div class="tw"><table><thead><tr><th>الصف</th><th>كود العميل</th><th>العميل</th><th>القطاع</th><th>الرصيد</th><th>الحالة</th></tr></thead><tbody>${body}</tbody></table></div>${plan.length>100?'<p class="sub">تظهر أول 100 صف؛ سيُعتمد كامل الملف.</p>':''}${pageInfo}${warning}`,async()=>{
      ...
      OPS.settings.customerOpeningBalanceImport={fileName:file.name,sourceHash:result.meta.sourceHash||'',sourceFormat:result.meta.format,reportDate:result.meta.reportDate,rowCount:plan.length,warningCount:result.meta.warnings.length,duplicatePageRows:result.meta.duplicatePageRows,ignoredPageRows:result.meta.ignoredPageRows,importedAt:stamp};
