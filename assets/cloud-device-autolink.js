(function(){
  ...
  // A device-transport cookie is not an administrator credential.  Older
  // builds copied a placeholder into the administrator-token slot, causing
  // protected calls to return a noisy 403 before the owner had logged in.
  // Keep real owner tokens intact, but remove only the legacy placeholder.
  function installPlaceholder(){try{if(localStorage.getItem(TOKEN_KEY)===PLACEHOLDER)localStorage.removeItem(TOKEN_KEY);}catch{}try{if((document.cookie.match(/(?:^|; )bh_cloud_token=([^;]*)/)||[])[1]===PLACEHOLDER)document.cookie='bh_cloud_token=; path=/; max-age=0; SameSite=Strict'+(location.protocol==='https:'?'; Secure':'');}catch{}}
  installPlaceholder();
