(function(){
  ...
  function ensureStyle(){
    ...
    style.id='bh-operations-actions-style';
    style.textContent=`
  ...
  function modal(title,html,kind='generic'){
    ...
    element.dataset.modalKind=kind;
  ...
  function addTaskButton(){
    ...
    button.onclick=()=>{
      ...
      dialog.querySelector('#bhTaskSave').onclick=async()=>{
        const saveButton=dialog.querySelector('#bhTaskSave');
        const resultBox=dialog.querySelector('#bhTaskResult');
        ...
        try{
          ...
          resultBox.innerHTML=`<div class="bh-ops-success">تم إنشاء المهمة <b>${esc(data.reference)}</b></div>`;
          setTimeout(()=>{
            dialog.remove();
          ...
          resultBox.innerHTML=`<div class="bh-ops-error">${esc(error.message)}</div>`;
        }
      };
    };
  ...
  function enhanceDetails(){
    ...
    for(const card of cards){
      if(card.dataset.actionsReady||!card.querySelector('.bh-ops-detail'))continue;
      const modalElement=card.closest('.bh-ops-modal');
      if(modalElement?.dataset.modalKind==='create-task')continue;
      ...
      box.innerHTML=`<textarea id="bhOperationNote" maxlength="2000" placeholder="ملاحظة التحديث"></textarea><label><input type="checkbox" id="bhOperationNotify" checked> إرسال التحديث إلى الموظف</label><div>${[
      ...
      box.querySelectorAll('[data-op-status]').forEach(statusButton=>{
        statusButton.onclick=async()=>{
          const buttons=[...box.querySelectorAll('[data-op-status]')];
          buttons.forEach(item=>item.disabled=true);
          try{
            await api({
              action:'set_status',
              reference,
              status:statusButton.dataset.opStatus,
  ...
  function addApprovalPanel(){
    ...
    fetch('/api/operations?limit=1',{
      ...
      .then(async response=>{
        const data=await response.json().catch(()=>({}));
        if(!response.ok)throw new Error(data.error||'تعذر تحميل الاعتمادات');
        return data;
      })
      .then(data=>{
        ...
        panel.querySelectorAll('[data-decision]').forEach(decisionButton=>{
          decisionButton.onclick=async()=>{
            const note=prompt(decisionButton.dataset.decision==='approved'?'ملاحظة الاعتماد':'سبب الرفض')||'';
            const actionButtons=[...panel.querySelectorAll('[data-decision]')];
            actionButtons.forEach(item=>item.disabled=true);
            try{
              await api({
                action:'approval_decision',
                id:decisionButton.dataset.id,
