...
(function(){
  ...
  function showMasterSync(masterSync){if(!masterSync)return;writeJson(MASTER_KEY,{...masterSync,at:new Date().toISOString()});if(masterSync.status==='delayed'){var pending=Number(masterSync.deferredChunks||0)+Number(masterSync.failedChunks||0);show('تم حفظ الحالة الأساسية، لكن مزامنة الجداول متأخرة في '+pending+' دفعة. ستُستكمل في الحفظ التالي.','warn',false);}else{show('تم حفظ الحالة واكتملت مزامنة الجداول الرئيسية.','ok',false);}window.dispatchEvent(new CustomEvent('binhamid-master-sync-status',{detail:masterSync}));}
