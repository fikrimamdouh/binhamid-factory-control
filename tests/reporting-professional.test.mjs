...
test('Telegram gateway handles approve, force-gap and reject callbacks',async()=>{
  ...
  assert.match(review,/if\(!id\|\|!canApprove\)return\{\}/);
});
...
test('timeline and from-to report page through every opening balance and posted movement',async()=>{
  const timeline=await read('api/_lib/daily-report-timeline.js');
  const report=await read('api/_lib/daily-customer-movement-pdf.js');
  for(const source of [timeline,report]){
    assert.match(source,/const PAGE_SIZE=1000/);
    assert.match(source,/offset=\$\{page\*PAGE_SIZE\}/);
  }
  assert.match(report,/rowsForBatches/);
  assert.match(report,/openingRowCount/);
  assert.match(report,/postedSalesRowCount/);
});
