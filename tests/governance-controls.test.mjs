...
test('migration workflow tests through schema 25 with encrypted backups and isolated restore',async()=>{
  ...
  assert.doesNotMatch(workflow,/\n  push:/);assert.match(workflow,/confirmation:/);assert.match(workflow,/APPLY_SCHEMA_25/);assert.match(workflow,/maintenance_window_confirmed/);assert.match(workflow,/Confirm protected maintenance window/);
