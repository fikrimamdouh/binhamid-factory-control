...
import { buildCustomerAnalytics, customerReportScope, findCustomers } from '../api/_lib/bot-customer-report-data.js';
...
test('customer analytics calculates balances, aging, credits and decision',()=>{
...
test('sales roles only see their customer sales scope',()=>{
  ...
  const block=buildCustomerAnalytics({...input,role:'block_sales'});
  const concrete=buildCustomerAnalytics({...input,role:'concrete_sales'});
...
test('customer lookup prioritizes exact code and supports Arabic name',()=>{
  const analytics=buildCustomerAnalytics({role:'admin',customers:[{external_id:'1001',customer_code:'1001',customer_name:'مصنع الأمل'}],sales:[{reference_no:'S1',sales_type:'block',customer_external_id:'1001',customer_name:'مصنع الأمل',total_amount:100,paid_amount:0,status:'registered',delivery_date:'2026-07-01'}],collections:[],asOf:new Date('2026-07-10T00:00:00Z')});
...
test('Telegram menu exposes customer reports and sourced product assistant',async()=>{
  const [enterprise,procurement,gateway,help]=await Promise.all([
    read('api/_lib/bot-enterprise.js'),read('api/_lib/bot-procurement-secure.js'),read('api/_lib/telegram-webhook-gateway.js'),read('api/_lib/bot-help.js')
  ]);
